...
/**
 * Like [`niceCeiling`], but the result is guaranteed to be strictly greater
 * than `value`: when the value already sits exactly on a 1–2–5×10ⁿ band
 * (e.g. 5 W, 10 W), it advances to the next band (5 → 10, 10 → 20). Used for
 * the adaptive power-gauge max so a reading equal to the observed peak never
 * pegs the arc at 100%.
 */
export function nextNiceCeiling(value: number): number | null {
  const ceil = niceCeiling(value)
  if (ceil === null) return null
  if (ceil > value) return ceil

  const magnitude = Math.pow(10, Math.floor(Math.log10(ceil)))
  const fraction = ceil / magnitude // 1, 2, 5 or 10
  if (fraction <= 1) return 2 * magnitude
  if (fraction <= 2) return 5 * magnitude
...
export function computePowerScale(
  ...
  if (limit !== null && limit > 0) {
    ...
    const ceiling = peak !== null ? nextNiceCeiling(peak) : null
