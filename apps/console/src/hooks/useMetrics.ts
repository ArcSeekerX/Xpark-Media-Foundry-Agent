...
import { generateMockMetrics, MOCK_ENABLED } from '../mock/metrics'
...
export function useMetrics() {
  ...
  const connect = useCallback(() => {
    ...
    ws.onmessage = (event) => {
      try {
        ...
        // The WS channel also carries typed WsEvent payloads (deploy/download
        // progress). Only accept actual metrics snapshots — treating an event
        // as a snapshot leaves `memory`/`disk` undefined and crashes the UI.
        if (
          !data ||
          typeof (data as unknown as { timestamp_ms?: unknown }).timestamp_ms !== 'number' ||
          !(data as unknown as { memory?: unknown }).memory ||
          !(data as unknown as { disk?: unknown }).disk
        ) {
          return
        }
  ...
  useEffect(() => {
    // Demo mode: synthesize snapshots so the console renders with no backend.
    if (MOCK_ENABLED) {
      setMetrics(generateMockMetrics())
      setConnectionStatus('connected')
      const id = setInterval(() => setMetrics(generateMockMetrics()), 2000)
