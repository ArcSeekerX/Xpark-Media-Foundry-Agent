...
/** Metric keys tracked per remote node (chart history for the detail view). */
const REMOTE_METRIC_KEYS = [
  'gpuUtil',
  'gpuTemp',
  'gpuPower',
  'gpuClock',
...
type RemoteMetricKey = (typeof REMOTE_METRIC_KEYS)[number]
...
function createRemoteBuffers(): Record<RemoteMetricKey, CircularBuffer<DataPoint>> {
  const buffers = {} as Record<RemoteMetricKey, CircularBuffer<DataPoint>>
  for (const key of REMOTE_METRIC_KEYS) {
    buffers[key] = new CircularBuffer<DataPoint>(BUFFER_CAPACITY)
  }
  return buffers
}

/** Push a possibly-null reading into a buffer. */
...
function extractGpuValue(gpu: MetricsSnapshot['gpu'] | undefined, key: MetricKey): number | null {
  if (!gpu) return null
  switch (key) {
    case 'gpuUtil':
...
function extractValue(metrics: MetricsSnapshot, key: MetricKey): number | null {
  switch (key) {
    ...
    case 'cpuAggregate':
      return metrics.cpu?.aggregate_percent ?? null
    case 'memoryUsedPercent':
      return metrics.memory?.total_bytes > 0
        ? (metrics.memory.used_bytes / metrics.memory.total_bytes) * 100
        : null
    case 'diskRead':
      return metrics.disk?.read_bytes_per_sec ?? null
    case 'diskWrite':
      return metrics.disk?.write_bytes_per_sec ?? null
    case 'networkRx':
      return metrics.network?.rx_bytes_per_sec ?? null
    case 'networkTx':
      return metrics.network?.tx_bytes_per_sec ?? null
  }
}
...
export function useMetricsHistory(
  ...
  >({})
  const remoteBuffersRef = useRef<
    Record<string, Record<RemoteMetricKey, CircularBuffer<DataPoint>>>
  >({})
  ...
  useEffect(() => {
    ...
    const gpus = metrics.gpus && metrics.gpus.length > 0 ? metrics.gpus : (metrics.gpu ? [metrics.gpu] : [])
    ...
    // Remote xpark cluster nodes: per-node chart history for the detail view.
    for (const node of metrics.remote_nodes ?? []) {
      if (!remoteBuffersRef.current[node.ip]) {
        remoteBuffersRef.current[node.ip] = createRemoteBuffers()
      }
      const rb = remoteBuffersRef.current[node.ip]
      const gpu = node.gpus[0]
      pushOptional(rb.gpuUtil, ts, gpu?.utilization_percent ?? null)
      pushOptional(rb.gpuTemp, ts, gpu?.temperature_celsius ?? null)
      pushOptional(rb.gpuPower, ts, gpu?.power_watts ?? null)
      pushOptional(rb.gpuClock, ts, gpu?.clock_graphics_mhz ?? null)
      pushOptional(rb.cpuAggregate, ts, node.cpu_percent ?? null)
      pushOptional(
        rb.memoryUsedPercent,
        ts,
        node.memory_total_bytes && node.memory_total_bytes > 0
          ? ((node.memory_used_bytes ?? 0) / node.memory_total_bytes) * 100
          : null,
      )
      pushOptional(rb.diskRead, ts, node.disk_read_bytes_per_sec ?? null)
      pushOptional(rb.diskWrite, ts, node.disk_write_bytes_per_sec ?? null)
      pushOptional(rb.networkRx, ts, node.network_rx_bytes_per_sec ?? null)
      pushOptional(rb.networkTx, ts, node.network_tx_bytes_per_sec ?? null)
    }
  ...
  /** Per-remote-node chart history (by node IP). */
  const getRemoteChartData = useCallback(
    (ip: string, metric: string): DataPoint[] => {
      void version
      ...
      const rb = remoteBuffersRef.current[ip]
      const buffer = rb?.[metric as RemoteMetricKey]
      if (!buffer) return []
