  ...
  if (engines.length === 0) {
    return (
      <Card className="bg-[#0d0d10] border-white/[0.04] h-full">
        <CardHeader>
          <CardTitle className="text-xl font-semibold text-zinc-100">推理引擎</CardTitle>
        </CardHeader>
        <CardContent className="py-8">
          <p className="text-zinc-100 text-center">未检测到推理引擎</p>
          <p className="text-zinc-500 text-sm text-center mt-2">
            启动 vLLM 推理引擎后，它将在数秒内自动显示在这里。
          </p>
        </CardContent>
      </Card>
    )
      ...
      <Card size="sm" className="bg-[#0d0d10] border-white/[0.04] overflow-hidden">
        <CardHeader className="flex flex-row justify-between items-center gap-4 min-w-0 shrink-0">
          <div className="shrink-0 flex items-center gap-4 min-w-0">
            {headerProviderLogo && (
              <div className="shrink-0 h-14 w-14 rounded-xl bg-[#fff] p-2 flex items-center justify-center ring-1 ring-white/[0.06]">
                <img
