...
export function EngineTab({ engine, cycle, intervalMs, showCountdown }: EngineTabProps) {
  ...
  const statusLabel = engine.status.type === 'Error'
    ? `错误: ${engine.status.message}`
    : engine.status.type === 'Running'
      ? '运行中'
      : engine.status.type === 'Loading'
        ? '加载中'
        : '已停止'
  const isStopped = engine.status.type === 'Stopped'
  const isDocker = engine.deployment_mode === 'Docker'
  const modeLabel = isDocker ? 'Docker' : '直接运行'
  ...
  return (
    ...
    >
      {providerLogo && (
        <span className="h-5 w-5 shrink-0 rounded bg-[#fff] p-0.5 flex items-center justify-center ring-1 ring-white/[0.06]">
          <img
